...
def netrank_loss(return_ratio, gt, mask, prev_w, tau, cost_bps, rho_concentration, alpha_t=None):
    ...
    # Apply dynamic scaling alpha_t to turnover penalty if provided
    if alpha_t is not None:
        alpha_t = alpha_t.squeeze()
        # The penalty is dynamically adjusted based on market state
        dynamic_turnover = turnover * alpha_t.mean() 
        net = gross - cost_rate * dynamic_turnover
    else:
...
def compute_tc_loss(prediction, gt, base_price, mask, prev_w, cfg, alpha_t=None):
    ...
    net_loss, w, net_stats = netrank_loss(
