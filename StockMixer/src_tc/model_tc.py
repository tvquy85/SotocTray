...
class StockMixerBackbone(nn.Module):
    def __init__(self, stocks, time_steps, channels, market, scale):
        super(StockMixerBackbone, self).__init__()
        scale_dim = 8
        ...
        # Dynamic NetRank context network
        self.context_net = nn.Sequential(
            nn.Linear(time_steps, 16),
            nn.GELU(),
            nn.Linear(16, 1)
        )
    ...
    def forward(self, inputs):
        ...
        # Calculate dynamic gamma multiplier alpha_t in [0.1, 1.0]
        # inputs shape: [batch, time_steps, stocks]
        market_state = inputs.mean(dim=2) # [batch, time_steps]
        alpha_t = 0.1 + 0.9 * torch.sigmoid(self.context_net(market_state))
