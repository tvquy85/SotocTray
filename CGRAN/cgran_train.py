...
from preprocess import market_state_from_closes_v1, market_state_from_closes_v2
from cgran_model import get_cgran_loss, CGRAN

# Evaluator port from parent directory if needed
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
try:
    from RGR_StockMixer.evaluator import evaluate
except ImportError:
    from GatedContext.evaluator import evaluate

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default='NASDAQ')
    parser.add_argument('--seed', type=int, default=123456789)
    parser.add_argument('--alpha', type=float, default=0.1, help="Weight for pairwise rank loss")
    parser.add_argument('--lambda_alpha', type=float, default=0.05, help="Weight for RA-Loss")
    parser.add_argument('--ctx_version', type=int, default=1, choices=[1, 2], help="1 for 5-dim context, 2 for 10-dim context")
...
def main():
    ...
    print(f"Extracting Macro Context v{args.ctx_version}...")
    if args.ctx_version == 1:
        market_ctx = market_state_from_closes_v1(eod_data, train_end_idx=valid_index)
    else:
        market_ctx = market_state_from_closes_v2(eod_data, train_end_idx=valid_index)
        
    ctx_dim = market_ctx.shape[1] # 5 or 10
    print(f"Context Extracted. Shape: {market_ctx.shape}, Dim: {ctx_dim}")
    ...
    def validate(start_index, end_index):
        ...
        with torch.no_grad():
            ...
            for cur_offset in range(
                ...
                prediction, alpha_pred = model(data_batch, ctx)
                cur_loss, cur_reg_loss, cur_rank_loss, cur_alpha_loss, cur_rr = get_cgran_loss(
                    prediction, alpha_pred, gt_batch, price_batch, mask_batch, stock_num, args
                )
    ...
    for epoch in range(args.epochs):
        ...
        for j in range(len(batch_offsets)):
            ...
            optimizer.zero_grad()
            prediction, alpha_pred = model(data_batch, ctx)
            cur_loss, cur_reg_loss, cur_rank_loss, cur_alpha_loss, _ = get_cgran_loss(
                prediction, alpha_pred, gt_batch, price_batch, mask_batch, stock_num, args
            )
            cur_loss.backward()
            optimizer.step()
        ...
        if val_loss < best_valid_loss:
            ...
            # Save best model
            torch.save(model.state_dict(), f"cgran_best_{args.dataset}_{args.seed}.pt")
        else:
            patience_counter += 1
