...
class GatingUnit(nn.Module):
    def __init__(self, dim, seq_len, ctx_dim=5):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.ln = nn.Sequential(
            nn.Linear(ctx_dim, dim // 2),
            nn.Hardswish(),
            nn.Linear(dim // 2, dim)
        )
...
class gMLPBlock(nn.Module):
    def __init__(self, input_dim, hidden_dim, seq_len, dropout_rate=0.0, ctx_dim=5):
        super().__init__()

        self.norm = nn.LayerNorm(input_dim)
        self.channel_proj1 = nn.Linear(input_dim, hidden_dim * 2)
        self.sgu = GatingUnit(hidden_dim, seq_len, ctx_dim=ctx_dim)
